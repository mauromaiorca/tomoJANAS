...
"""
    ...
    # Include the current best subset so its locres is measured under this iteration's mask
    best_reference_numParticles=$(janas_optimizer getTarget \
        --overviewFile "${workingDir}/overview.txt" --reference_num_particles 2>/dev/null || true)
    if [ -n "$best_reference_numParticles" ] && [ "$best_reference_numParticles" != "None" ] && [ "$best_reference_numParticles" != "" ]; then
        listParticles="${best_reference_numParticles},${listParticles}"
    fi

    # Deduplicate while preserving order
    listParticles=$(printf '%s' "$listParticles" | tr ',' '\n' | awk 'NF && !seen[$0]++' | paste -sd, -)
    echo "listParticles_dedup=$listParticles" >>${workingDir}/${tag}/log.txt

    # Ensure locres is computed over the union of the main mask and any assess mask,
    # so that the assessment region never falls outside the local-resolution map.
    mask_for_locres="$targetMask"
    if [ -n "${assessMask}" ] && [ "${assessMask}" != "None" ] && [ -f "${assessMask}" ]; then
        mask_for_locres="${workingDir}/${tag}/merged_locres_mask.mrc"
        run_step "${ite}" "04b_merge_masks" "${workingDir}/${tag}/steps" \
            janas_utils mask merge "$targetMask" "${assessMask}" "${mask_for_locres}"
    fi
    ...
    if [ "$(str2bool "$USE_EXTERNAL_TOOLS")" -eq "$True" ]; then
        ...
        run_step "${ite}" "06_locres_bulk" "${workingDir}/${tag}/steps" \
            janas_utils locresBulk --threshold 0.143 --gamma 1.5 --cycles 15 --cpu """ + numMpi + """ --mask ${mask_for_locres} ${workingDir}/${tag}/norm_${tag}_best $listParticles
    fi
    ...
    useAdaptiveMask=$(str2bool "$adaptiveMask")
    if [ "$useAdaptiveMask" -eq "$True" ]; then
        run_step "${ite}" "07_locres_stats_fullmask" "${workingDir}/${tag}/steps" \
            janas_utils locresStats --locres-files "${workingDir}/${tag}/norm_${tag}_best*locres.mrc" --mask ${mask_for_locresStats} --assessmentMethod ${assessmentMethod} --out-stats ${workingDir}/${tag}/bestRanked_locres_values_fullMask.csv --out-local-best-particles ${workingDir}/${tag}/partialLocres.mrc
        run_step "${ite}" "07a_adaptive_mask" "${workingDir}/${tag}/steps" \
            janas_utils split_mask --map ${workingDir}/${tag}/partialLocres.mrc --mask ${mask_for_locresStats} --blur ${adaptiveMaskLocresBlur} --o_below ${workingDir}/${tag}/adaptive_maskRaw.mrc

        # Decide whether to merge with target adaptive mask or use raw mask directly
        useIndependent=$(str2bool "$adaptiveMaskIndependent")
        if [ "$useIndependent" -eq "$True" ]; then
            # Independent mode: use raw mask directly
            cp -f "${workingDir}/${tag}/adaptive_maskRaw.mrc" "${workingDir}/${tag}/adaptive_mask.mrc"
            echo "Adaptive mask: using independent raw mask (--adaptive_mask_independent requested)." >> "${workingDir}/${tag}/log.txt"
        else
            # Growing mode (default): try to merge with target adaptive mask
            targetAdaptiveMask=$(janas_optimizer getTarget --overviewFile "${workingDir}/overview.txt" --assessment_mask_file 2>/dev/null || true)
            if [ -n "${targetAdaptiveMask}" ] && [ "${targetAdaptiveMask}" != "None" ] && [ "${targetAdaptiveMask}" != "" ] && [ -f "${targetAdaptiveMask}" ]; then
                run_step "${ite}" "07a_merge_adaptive_mask" "${workingDir}/${tag}/steps" \
                    janas_utils mask merge "${targetAdaptiveMask}" "${workingDir}/${tag}/adaptive_maskRaw.mrc" "${workingDir}/${tag}/adaptive_mask.mrc"
                echo "Adaptive mask: merged raw mask with target adaptive mask (${targetAdaptiveMask})." >> "${workingDir}/${tag}/log.txt"
            else
                # No valid target adaptive mask available: use raw mask
                cp -f "${workingDir}/${tag}/adaptive_maskRaw.mrc" "${workingDir}/${tag}/adaptive_mask.mrc"
                echo "Adaptive mask: no previous target adaptive mask available, using raw mask." >> "${workingDir}/${tag}/log.txt"
            fi
        fi

        # Small-mask safeguard (applies to both merged and independent masks)
        adaptiveMaskCount=$(janas_utils mask countPixels "${workingDir}/${tag}/adaptive_mask.mrc")
        if [[ "${adaptiveMaskCount}" =~ ^[0-9]+$ ]] && [ "${adaptiveMaskCount}" -le 10 ]; then
            echo "Adaptive mask too small (${adaptiveMaskCount} voxels). Replacing with full assessment mask." >> "${workingDir}/${tag}/log.txt"
            cp -f "${mask_for_locresStats}" "${workingDir}/${tag}/adaptive_mask.mrc"
        fi
        run_step "${ite}" "07b_locres_stats_local" "${workingDir}/${tag}/steps" \
            janas_utils locresStats --locres-files "${workingDir}/${tag}/norm_${tag}_best*locres.mrc" --mask ${workingDir}/${tag}/adaptive_mask.mrc --assessmentMethod ${assessmentMethod} --out-stats ${workingDir}/${tag}/bestRanked_locres_values.csv
    else
...
"""
    ...
    # ------------------------------------------------------------------
    # Emit the ad-hoc subset extractor for this iteration.
    # The script is always written to the iteration directory; it is only
    # promoted to the session root when this iteration becomes the new
    # target selection (the "current best" reflected in overview.txt).
    # See docs/custom_selected_stacks.md for context.
    # ------------------------------------------------------------------
    local extractScript="${workingDir}/${tag}/extract_custom_selected_stack.sh"
    cat > "${extractScript}" <<EXTRACT_EOF
#!/bin/bash
# Generated by janas_session_manager.
# Extract the top-N best-ranked particles from this selection iteration
# and produce an Euler-angle histogram + a small manifest.
#
# Usage: ./extract_custom_selected_stack.sh <N>
#   N : number of best-ranked particles to extract (mandatory).
#
# Output: custom_selected_stacks/subset_<N>_ite<ITE>/ (relative to the
# session directory) containing the STAR file, the Euler histogram PNG
# and a manifest.txt with the provenance of the subset.
set -euo pipefail

if [ \$# -lt 1 ]; then
    echo "Usage: \$(basename "\$0") <N>" >&2
    echo "  N : number of best-ranked particles to extract (mandatory)." >&2
    exit 2
fi
N="\$1"
...
# The script lives either inside the iteration directory or at the session
# root (once promoted as the current best). Resolve the session directory
# accordingly so the same script works in both locations.
if [ "\$(basename "\${SCRIPT_DIR}")" = "\${TAG}" ]; then
    SESSION_DIR="\$(dirname "\${SCRIPT_DIR}")"
else
    SESSION_DIR="\${SCRIPT_DIR}"
fi

SOURCE_STAR="\${SESSION_DIR}/\${TAG}/scored_selection_\${ITE}.star"
if [ ! -f "\${SOURCE_STAR}" ]; then
    echo "ERROR: scored selection STAR not found: \${SOURCE_STAR}" >&2
    exit 3
fi

OUT_DIR="\${SESSION_DIR}/custom_selected_stacks/subset_\${N}_ite\${ITE}"
mkdir -p "\${OUT_DIR}"
OUT_STAR="\${OUT_DIR}/subset_\${N}_ite\${ITE}.star"
OUT_HIST="\${OUT_DIR}/subset_\${N}_ite\${ITE}_eulerhist.png"
OUT_MANIFEST="\${OUT_DIR}/subset_\${N}_ite\${ITE}_manifest.txt"

# --exact: fail explicitly if the source STAR has fewer particles than N
janas selectBestRanked --i "\${SOURCE_STAR}" --num "\${N}" --exact \\
    --o "\${OUT_STAR}"

janas eulerHist --i "\${OUT_STAR}" --outImage "\${OUT_HIST}" \\
    --show False --fontScale 2.0

cat > "\${OUT_MANIFEST}" <<MANIFEST_EOF
JANAS custom selected stack
  N requested:    \${N}
  iteration:      \${ITE}
  iteration tag:  \${TAG}
  source STAR:    \${SOURCE_STAR}
  output STAR:    \${OUT_STAR}
  euler hist:     \${OUT_HIST}
...
EXTRACT_EOF
    ...
    # Promote to session root only when this iteration is the new current best.
    current_target_ID=$(janas_optimizer getTarget --overviewFile "${workingDir}/overview.txt" --current_selection_ID 2>/dev/null || echo "")
    if [ -n "${current_target_ID}" ] && [ "${current_target_ID}" = "${ite}" ]; then
        cp -f "${extractScript}" "${workingDir}/extract_custom_selected_stack.sh"
        chmod +x "${workingDir}/extract_custom_selected_stack.sh"
    fi
...
"""
    ...
    run_script_cmd += f'adaptiveMask="{as_str(data.get("adaptiveMask", "False"))}"\n'
    run_script_cmd += f'adaptiveMaskLocresBlur="{as_str(data.get("adaptiveMaskLocresBlur", "1.0"))}"\n'
    run_script_cmd += f'adaptiveMaskIndependent="{as_str(data.get("adaptiveMaskIndependent", "False"))}"\n'
...
masks.add_argument(
    ...
    action="store_true",
    help=(
        "Build an adaptive assessment mask from partialLocres.mrc and use it "
        "for the final locresStats evaluation of each selection."
    ),
)
masks.add_argument(
    "--adaptive_mask_locres_blur",
    required=False,
    type=float,
    default=1.0,
    help=(
        "Gaussian sigma used by janas_utils split_mask when deriving the "
        "adaptive assessment mask from partialLocres.mrc."
    ),
)
masks.add_argument(
    "--adaptive_mask_independent",
    action="store_true",
    help=(
        "Force the current iteration to use an independent adaptive mask "
        "without merging it with the target selection adaptive mask. "
        "Implies --adaptive_mask even if not explicitly provided."
    ),
)
...
def new_select_session(args):
    ...
    with open(settings_file_path, "w") as file:
        ...
        file.write("\n# Optional adaptive assessment mask derived from partialLocres.mrc\n")
        # --adaptive_mask_independent implies adaptive masking
        effective_adaptive_mask = bool(getattr(args, "adaptive_mask", False)) or bool(getattr(args, "adaptive_mask_independent", False))
        adaptive_mask_independent = bool(getattr(args, "adaptive_mask_independent", False))
        file.write(f'adaptiveMask = "{str(effective_adaptive_mask)}"\n')
        file.write(f'adaptiveMaskLocresBlur = "{float(getattr(args, "adaptive_mask_locres_blur", 1.0))}"\n')
        file.write(f'adaptiveMaskIndependent = "{str(adaptive_mask_independent)}"\n')

        file.write("\n# How the optimizer judges improvement across iterations:\n")
        file.write('#   "resolution" — compare resolution values (requires fixed mask)\n')
        file.write('#   "target_stability" — check if selected subset changed (for adaptive masking)\n')
        file.write('#   "auto" — infer from adaptiveMask setting (default)\n')
        file.write('comparison_mode = "auto"\n')
