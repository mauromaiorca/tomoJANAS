...
### Added (newest)
- **Axis order is explicit, default `xyz`** (no header-based auto-detection): the three input coordinate numbers map directly to the rec file's X, Y, Z axes. Pass `--axis-order xzy` for a flipped IMOD tomogram (thickness on the middle axis) or `--axis-order zyx` for napari single points. (An earlier auto-detection/`[auto]`/`[warn]` mechanism was removed in favour of predictable, explicit control.)
