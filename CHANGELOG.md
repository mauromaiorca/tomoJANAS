...
### Added (newest)
- **Automatic axis-order detection** for rec-voxel picks: `--axis-order auto` (the default) now reads the reconstructed tomogram header and chooses `xyz` (natural orientation, thickness = last axis) or `xzy` (flipped raw IMOD `tilt` output, thickness = middle axis). Pasting coordinates from the IMOD 3dmod Zap window now Just Works without specifying `--axis-order`. The decision is printed (`[auto] …`) and logged.
