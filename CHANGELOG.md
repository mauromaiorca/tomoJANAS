...
### Added (newest)
- **Axis order is explicit, default `xyz`** (no header-based auto-detection): the three input coordinate numbers map directly to the rec file's X, Y, Z axes — the standard mapping, and what IMOD 3dmod coordinates use (paste them as `x,y,z`). Only pass a different `--axis-order` when your coordinates are genuinely in another order (e.g. `zyx` for napari single points). (An earlier auto-detection/`[auto]`/`[warn]` mechanism was removed in favour of predictable, explicit control.)
- **All six axis-order permutations** (`xyz, xzy, yxz, yzx, zxy, zyx`) and **per-axis coordinate flips** (`--coord-flip-x/y/z`, invert `v → (N-1)-v` in 0-based rec-voxel space) for tools that count an axis from the end. Indexing + axis-order + flips are resolved once into a canonical 0-based rec-voxel coordinate used for the centered-Ångström value, projections and the crop.
